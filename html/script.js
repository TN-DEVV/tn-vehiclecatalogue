...
function menuVehicle(event) {
  ...
  $.post(
    "https://tn-vehiclecatalogue/menuSelected",
    JSON.stringify({ menuId: categoryVehicleSelected.toLowerCase() })
  );
}

function testDrive() {
  $.post(
    "https://tn-vehiclecatalogue/TestDrive",
    JSON.stringify({ vehicleModel: vehicleSelected.modelcar })
  );
      ...
      `</span></p>
        ...
        <div class="modal-footer">
            <div class="modal-price">
                <p class='price sale'>$ ` +
      vehicleSelected.sale / 1000 +
      ` k</p>
                <p class='price discount'>$ ` +
      parseFloat((vehicleSelected.sale / 1000) * 1.1).toFixed(3) +
      ` k</p>                  
            </div>
...
function buyVehicle(option) {
  ...
  switch (option) {
    ...
    case "confirm":
      $.post(
        "https://tn-vehiclecatalogue/Buy",
        JSON.stringify(vehicleSelected)
      );
      break;
  }
}
...
$(document).on("keydown", function () {
  switch (event.keyCode) {
    case 27: // ESC
      $.post("https://tn-vehiclecatalogue/Close");
      $("body").css("display", "none");
      document.getElementById("top-menu").innerHTML = "";
      break;
    case 9: // TAB
    ...
    case 68: // LEFT A
      $.post(
        "https://tn-vehiclecatalogue/rotate",
        JSON.stringify({ key: "left" })
      );
      break;
    case 65: // RIGHT D
      $.post(
        "https://tn-vehiclecatalogue/rotate",
        JSON.stringify({ key: "right" })
      );
      break;
    case 39:
...
$(document).on("keydown", function (ev) {
  ...
  if (_key) {
    $.post("https://tn-vehiclecatalogue/rotate", JSON.stringify({ key: _key }));
  }
});
  ...
  switch (event.which) {
    ...
    case 1:
      ...
      if (!itemDisabled) {
        ...
        document.getElementById("vehiclebrand").innerHTML =
          ' <img id="vehicle_brand_image" src="../imgs/brands/' +
          dataCar[0].outerText +
          '.png">';
        ...
        $.post(
          "https://tn-vehiclecatalogue/SpawnVehicle",
          JSON.stringify({
            modelcar: dataCar[9].outerText,
...
function setVehicleColorRGB(R, G, B) {
  if (selectedColor == "primary") {
    $.post(
      "https://tn-vehiclecatalogue/RGBVehicle",
      JSON.stringify({ primary: true, R: R, G: G, B: B })
    );
  } else {
    $.post(
      "https://tn-vehiclecatalogue/RGBVehicle",
      JSON.stringify({ primary: false, R: R, G: G, B: B })
    );
  }
}
